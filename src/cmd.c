...
void cmd_psize(unsigned char argc, char *argv[]) reentrant {
    if (argc < 2) {
        UART1_send_string("Usage: psize -b|-s\r\n");
        return;
    }
...
void cmd_enable(unsigned char argc, char *argv[]) __reentrant {
    if (argc < 2) {
        UART1_send_string("Usage: enable -1|-2\r\n");
        return;
    }
...
void cmd_disable(unsigned char argc, char *argv[]) reentrant {
    if (argc < 2) {
        UART1_send_string("Usage: disable -1|-2\r\n");
        return;
    }
    if (strcmp(argv[1], "-1") == 0) {
...
const unsigned char cli_cmd_count = sizeof(cli_commands);
